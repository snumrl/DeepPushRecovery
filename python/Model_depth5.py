...
class SimulationNN(nn.Module):
    def __init__(self,num_states,num_actions):
        ...
        self.p_fc4 = nn.Linear(num_h3,num_h4)
        self.p_fc5 = nn.Linear(num_h4,num_h5)
        self.p_fc6 = nn.Linear(num_h5,num_actions)
        self.log_std = nn.Parameter(torch.zeros(num_actions))

        self.v_fc1 = nn.Linear(num_states,num_h1)
        self.v_fc2 = nn.Linear(num_h1,num_h2)
        self.v_fc3 = nn.Linear(num_h2,num_h3)
        self.v_fc4 = nn.Linear(num_h3,num_h4)
        self.v_fc5 = nn.Linear(num_h4,num_h5)
        self.v_fc6 = nn.Linear(num_h5,1)
        ...
        torch.nn.init.xavier_uniform_(self.p_fc5.weight)
        torch.nn.init.xavier_uniform_(self.p_fc6.weight)

        self.p_fc1.bias.data.zero_()
        self.p_fc2.bias.data.zero_()
        self.p_fc3.bias.data.zero_()
        self.p_fc4.bias.data.zero_()
        self.p_fc5.bias.data.zero_()
        self.p_fc6.bias.data.zero_()
        ...
        torch.nn.init.xavier_uniform_(self.v_fc5.weight)
        torch.nn.init.xavier_uniform_(self.v_fc6.weight)

        self.v_fc1.bias.data.zero_()
        self.v_fc2.bias.data.zero_()
        self.v_fc3.bias.data.zero_()
        self.v_fc4.bias.data.zero_()
        self.v_fc5.bias.data.zero_()
        self.v_fc6.bias.data.zero_()

    def forward(self,x):
        p_out = F.relu(self.p_fc1(x))
        p_out = F.relu(self.p_fc2(p_out))
        p_out = F.relu(self.p_fc3(p_out))
        p_out = F.relu(self.p_fc4(p_out))
        p_out = F.relu(self.p_fc5(p_out))
        p_out = self.p_fc6(p_out)
        ...
        v_out = F.relu(self.v_fc4(v_out))
        v_out = F.relu(self.v_fc5(v_out))
        v_out = self.v_fc6(v_out)
        return p_out,v_out
