...
from main_PrintBvh import getMotionFromMotionSkel
...
def worker_simulation(sim, param, csvfile, option_str=''):
    ...
    if stopcode != 1:
        path_prefix = '/mass/results/result_motion/'
        if not os.path.exists(path_prefix):
            os.makedirs(path_prefix)
        filename_prefix = 'nopush_' if 'nopush' in option_str else 'push_'
        filename_prefix += 'ad_' if 'ad' in option_str else 'bf_'
        filename_prefix += 'dep3_' if 'dep3' in option_str else ''
        if push_force > 0:
            filename = filename_prefix+str(trial_angle)+'deg_'+str(push_force)+'N_'+str(push_start_timing)+'t_step_'+str(step_length)[:5]+'_speed_'+str(walking_speed)[:5]
        else:
            filename = filename_prefix+str(trial_angle)+'deg_step_'+str(step_length)[:5]+'_speed_'+str(walking_speed)[:5]
...
def simulate(sim, launch_order, num=100, option_str='', trial_force=None):
    ...
    path_prefix = os.path.dirname(os.path.abspath(__file__)) + '/results/result_motion/'
    if not os.path.exists(path_prefix):
        os.makedirs(path_prefix)
    ...
    for i in range(len(paramgroups)):
        for j in range(len(paramgroups[i])):
            worker_simulation(sim, paramgroups[i][j], csvfile, option_str)
        print()
